...
namespace Gadgetron
{
    namespace
    {
                                                      ...
                                                      float girf_sampling_time, float TE)
        {
            ...
            size_t nlines_new = girf_kernel.get_size(0) * girf_sampling_time / gradient_sampling_time;
            ...
            for (int batch = 0; batch < nbatches; batch++)
            {
                ...
                for (size_t k = 0; k < 3; k++)
                {
                    for (size_t i = nlines; i < nlines + filter.size() / 2 + 1; i++)
                    {
                        rotated(i, k) = filter[filter.size() / 2 + 1 + i - nlines] * (rotated(nlines - 1, k)); // last term for fft scaling
                    }
    ...
    hoNDArray<std::complex<float>> GIRF::zeropadding(hoNDArray<std::complex<float>> input, int zpadFactor)
    {
        using namespace Gadgetron::Indexing;
