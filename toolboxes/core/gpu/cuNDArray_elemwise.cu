...
template <typename T>
  struct cuNDA_real_imag_to_complex : public thrust::binary_function<typename realType<T>::Type, typename realType<T>::Type, T>
  {
  ...
  template <typename T>
  struct cuNDA_imag_to_complex : public thrust::unary_function<typename realType<T>::Type, T>
  {
    __device__ T operator()(const typename realType<T>::Type &x) const { return T(realType<T>::Type(0), x); }
  };
